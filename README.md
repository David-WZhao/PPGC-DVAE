...
The dataset used in this paper is located in the `data/` directory. The `train_last.csv` is the training set, `valid_last.csv` is the validation set, and `test_last.csv` is the test set. These three files have been processed and are ready for use; you can download them for training directly.
...
1. Unconditional generation
   ...
   ```
   python generate_10000.py
   ```

   This code is designed to simulate unconditional generation. The input conditions are property conditions sampled from a normal distribution, and the number of generated AMPs can be set freely.
